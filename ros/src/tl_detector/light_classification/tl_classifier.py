...
class TLClassifier(object):
    ...
    def train_site(self):
        model = self.create_model(4)
        model.compile(optimizer='rmsprop', loss='categorical_crossentropy', metrics=['accuracy'])
        train_datagen = ImageDataGenerator(
            preprocessing_function=self.preprocess_site)
    ...
    def test(self):
        model = load_model('src/tl_detector/model.h5')
        test_datagen = ImageDataGenerator(
            preprocessing_function=self.preprocess_input
        )
        test_generator = test_datagen.flow_from_directory(
            'src/tl_detector/light_classification/real_lights',
            target_size=(IMG_WIDTH, IMG_HEIGHT),
            batch_size=8,
            class_mode='categorical')
